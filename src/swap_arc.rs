...
pub struct SwapArcIntermediateTLS<T, D: DataPtrConvert<T> = Arc<T>, const METADATA_HEADER_BITS: u32 = 0> {
    pub(crate) curr_ref_cnt: AtomicUsize, // the last bit is the `update` bit
    ptr: AtomicPtr<T>,
    pub(crate) intermediate_ref_cnt: AtomicUsize, // the last bit is the `update` bit
...
impl<T, D: DataPtrConvert<T>, const METADATA_PREFIX_BITS: u32> SwapArcIntermediateTLS<T, D, METADATA_PREFIX_BITS> {
    ...
    pub unsafe fn load_raw_full(self: &Arc<Self>) -> SwapArcIntermediateFullPtrGuard<T, D, METADATA_PREFIX_BITS> {
        ...
        SwapArcIntermediateFullPtrGuard {
            parent: ManuallyDrop::new(D::from(Self::strip_metadata(guard.ptr))).deref().clone(),
            ptr: guard.ptr,
        }
    }
    ...
    pub(crate) unsafe fn update_raw(&self, updated: *const T) {
    ...
    pub(crate) unsafe fn try_compare_exchange_with_meta(&self, old: *const T, new: *const T) -> bool {
        // let mut back_off_weight = 1;
        println!("entered cmp exchg!");
        let backoff = Backoff::new();
        let mut tmp = self.intermediate_ref_cnt.compare_exchange(0, Self::UPDATE | Self::OTHER_UPDATE, Ordering::SeqCst, Ordering::Relaxed);
        while !tmp.is_ok() {
            println!("tmp ref_cnt: {} | curr: {}", tmp.err().unwrap(), self.curr_ref_cnt.load(Ordering::Acquire));
            // back-off
            /*thread::sleep(Duration::from_micros(10 * back_off_weight));
            back_off_weight += 1;*/
            backoff.snooze();
            tmp = self.intermediate_ref_cnt.compare_exchange(0, Self::UPDATE | Self::OTHER_UPDATE, Ordering::SeqCst, Ordering::Relaxed);
        }
        let intermediate = self.intermediate_ptr.load(Ordering::Acquire);
